...
//	GPIO_ResetBits(GPIOB, HC595_RCLK);
	...
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_2MHz;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
	GPIO_InitStructure.GPIO_Pin = HC595_DATA|HC595_SCLK|HC595_RCLK;
	GPIO_Init(GPIOB, &GPIO_InitStructure);

	GPIO_ResetBits(GPIOB, HC595_DATA|HC595_SCLK|HC595_RCLK);
}
...
void hc595_write_byte(uint8_t byte)
{
	...
	for (i=0; i<8; i++)
	{
		GPIO_ResetBits(GPIOB, HC595_SCLK);
		if ((num&0x01)==0x01)
			GPIO_SetBits(GPIOB, HC595_DATA);
		else
			GPIO_ResetBits(GPIOB, HC595_DATA);
        Delay_us(1);
		GPIO_SetBits(GPIOB, HC595_SCLK);
		num = num>>1;
	}
    GPIO_ResetBits(GPIOB, HC595_RCLK);
